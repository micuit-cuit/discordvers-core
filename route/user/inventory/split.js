...
const log = new LOG([{ style: 'square', color: 'red', text: 'discrod-vers-core' }, { style: 'circle', color: '#ff00ff', text: 'user' }, { style: 'circle', color: 'yellow', text: 'inventory' }, { style: 'circle', color: 'blue', text: 'splitInventory' }]);
/* 
    @route: /user/inventory/split
    @name: splitInventory
    @description: Split an item from the inventory
    @param: discordUserID, slotFrom, slotTo, quantity )
...
module.exports = function (db, ws, userID, args, requestID) {
    ...
    let { slotFrom, slotTo, quantity = 2048 } = args;
    if (!slotFrom || !slotTo) {
        log.e('missing slot');
    ...
    if (!slotFrom === slotTo) {
        log.e('slotFrom and slotTo are the same');
        ws.send(JSON.stringify({ success: false, data: 'slotFrom and slotTo are the same', requestID }));
        return;
    }
    ...
    log.d('splitping item from', slotFrom, 'to', slotTo, 'quantity:', quantity);
    ...
    userDB.findOne({ where: { id: userID } }).then((user) => {
        ...
        inventoryDB.findOne({ where: { id: user.inventory } }).then(async (inventory) => {            //pour chaque slot de l'inventaire, récupère l'item correspondant (les 20 slots en même temps)
            ...
            itemDB.findOne({ where: { id: item } }).then(async (item) => {
                //si le nombre d'item à diviser est supérieur au nombre d'item dans le slot, on divise par 2
                if (item.quantity < 2) {
                    log.e('item quantity is less than 2');
                    ws.send(JSON.stringify({ success: false, data: 'item quantity is less than 2', requestID }));
                    return;
                }
                ...
                if (inventory[slotTo]) {
                    log.e('slotTo is not empty');
                    ws.send(JSON.stringify({ success: false, data: 'slotTo is not empty', requestID }));
                    return;
                }
                //on divise la quantité de l'item par 2
                item.quantity -= quantity;
                //on crée un nouvel item avec la moitié de la quantité
                itemDB.create({
                    ...
                    inventory[slotTo] = newItem.id;
                    //sauvegarde l'inventaire
                    await inventory.save()
                    await item.save();
                            
                    log.s('item splitped from', slotFrom, 'to', slotTo, 'quantity:', quantity);
                    ...
                    log.e('error spliting item:', err);
                    ws.send(JSON.stringify({ success: false, data: 'error spliting item', requestID }));
                });
            }).catch((err) => {
